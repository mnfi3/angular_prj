...
const routes: Routes = [
    ...
    },
    {
        ...
        data: {willSetSeo: true, parentHomeRoute: true, permissions: ['tracks.view']},
        children: [
            {
                path: '',
                component: ChannelShowComponent,
                resolve: {api: ChannelResolverService},
                data: {name: 'channel', channelName: 'discover'}
            },
            {
                path: 'user',
                loadChildren: () => import('app/web-player/users/user-profile-page/user-profile.module')
                    .then(m => m.UserProfileModule)
            },
            {
                path: 'upload',
                data: {permissions: ['tracks.create', 'albums.create']},
                loadChildren: () => import('app/uploading/uploading.module')
